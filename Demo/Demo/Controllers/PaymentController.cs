...
public class PaymentController : Controller
{
    ...
    //    if (payment == null)
    //    {
    //        return new JsonResult(new { status = "error", message = "payment is null" });
    //    }

    //    if (string.IsNullOrEmpty(payment?.PaypalCaptureId))
    //    {
    //        return new JsonResult(new { status = "error", message = "capture id not found" });
    //    }
    ...
    //                redirectUrl = Url.Action("OrderDetail", "Product", new { id = order.OrderID })
    //            });
    //        }
    //        else
    //        {
    ...
    [HttpPost]
    public async Task<IActionResult> RefundPaypalOrder(int orderId)
    {
        var order = db.Orders.FirstOrDefault(o => o.OrderID == orderId && o.Paid);

        if (order == null)
        {
            TempData["Error"] = "Order not found or not paid";
            return Redirect(Request.Headers["Referer"].ToString());
        }

        var payment = db.Payments.FirstOrDefault(p => p.OrderID == orderId);

        if (payment == null || string.IsNullOrEmpty(payment.PaypalCaptureId))
        {
        ...
        using (var client = new HttpClient())
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                TempData["Info"] = "Refund completed successfully";
            }
            else
            {
                TempData["Error"] = "PayPal refund failed";
            }
        }
